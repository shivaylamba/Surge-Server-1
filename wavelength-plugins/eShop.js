...
function shopDisplay() {
	let output = '<div style="max-height:300px; width: 100%; overflow: scroll"><table style="border:1px solid #000; border-radius: 4px; width: 100%;"><center><font color="#3399FF"><font size="4"></u><b> Surge Shop</b></u></font></font></center><br>';
	for (let i in WL.eShop) {
		if (!WL.eShop[i]) continue;
		output += '<tr><td style="border: 1px solid #000; width: 20%; text-align: center"><button name="send" style="background: #3399FF; border: 1px #000 solid" value="/shop buy ' + WL.eShop[i].id + '">' + WL.eShop[i].name + '</button></td><td style="border: 1px solid #000; width: 70%; text-align: center">' + WL.eShop[i].desc + '</td><td style="border: 1px solid #000; width: 10%; text-align: center">' + WL.eShop[i].price + '</td></tr>';
	}
	output += '</table></div>';
	return output;
}
