...
WL.showNews = function (userid, user) {
	...
	if (newsDisplay.length > 0) {
		...
		return user.send(`|pm| Surge Server|${user.getIdentity()}|/raw ${newsDisplay}`);
	}
};
...
exports.commands = {
	...
	serverannouncements: {
		...
		view: function (target, room, user) {
			...
			let output = `<center><strong>Surge News:</strong></center>${generateNews().join(`<hr>`)}${showSubButton(user.userid)}`;
