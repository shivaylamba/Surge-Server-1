...
function loadShops() {
	try {
		WL.shopData = JSON.parse(fs.readFileSync('config/shopData.json', 'utf8'));
	} catch (e) {
		WL.shopData = {
			closed: true
		};
	}
	if (!WL.shopData.closed) buildShop(WL.shopData);
}
setTimeout(function() {
	loadShops();
}, 2000);
...
function getItems(obj, objname) {
	...
	for (let i in properties) {
		...
		if (obj[name].items) {
			...
			WL.itemList.push(shopItem);
			if (WL.currencies.indexOf(shopItem.currency) === -1) WL.currencies.push(shopItem.currency);
		}
	}
}

function drawMain(output) {
	let shopData = WL.shopData
...
function assembleOutput(output, marquee, shop, selectionType, back, update, user, room, cache) {
	...
	shop = '<div style="padding: 5px; border: 2px solid #101ad1; background: #6600CC; color: #fff; text-shadow: none; border-radius: 6px; margin: 10px"><center><font size=4><i><b>Surge Server Shop</b></i></font></center></div><br/><br/>' + output;
...
exports.commands = {
	...
	redeem: function(target, room, user) {
		...
		let userPacks;
		for (let i in WL.itemList) {
			if (WL.itemList[i].name == target) match = WL.itemList[i];
		}
		...
		Economy.readMoney(user.userid, stardust => {
			switch (match.name) {
				...
				case 'globaldeclare':
					...
					success = runTransaction(money, match, user);
					if (success) {
						//WL.messageSeniorStaff(user.name + " has purchased a Global Declare. Please contact this user to run their Global Declare.");
				...
				case 'customcolor':
					...
					success = runTransaction(money, match, user);
					if (success) {
						//WL.messageSeniorStaff(user.name + " has purchased a Custom Color. Please contact this user to setup their Custom Color.");
				...
				case 'title':
					...
					success = runTransaction(money, match, user);
					if (success) {
						//WL.messageSeniorStaff(user.name + " has purchased a Custom Title. Please contact this user to setup their Custom Color.");
				...
				case 'emote':
					...
					success = runTransaction(money, match, user);
					if (success) {
						//WL.messageSeniorStaff(user.name + " has purchased an Emote. Please contact this user to setup their Emote.");
				...
				case 'profileteam':
					...
					//SSBFFA
				case 'shiny':
					if (!WL.ssb[user.userid]) return this.sendReply('You need to run /ssb edit at least once before you can buy this.');
				...
				case 'customitem':
					...
					if (success) {
						...
						writeSSB();
						WL.messageSeniorStaff(user.name + " has purchased a Custom Item. Please contact this user to get details on what custom item they want.");
						successfulTransaction(match, success, user, room);
					} else {
						failedTransaction(user, match, money, room);
					}
				...
				case 'customability':
					...
					if (success) {
						...
						writeSSB();
						WL.messageSeniorStaff(user.name + " has purchased a Custom Ability. Please contact this user to get details on what custom ability they want.");
						successfulTransaction(match, success, user, room);
					} else {
						failedTransaction(user, match, money, room);
					}
				...
				case 'custommove':
					...
					if (success) {
						...
						writeSSB();
						WL.messageSeniorStaff(user.name + " has purchased a Custom Move. Please contact this user to get details on what custom move they want.");
						successfulTransaction(match, success, user, room);
					} else {
						failedTransaction(user, match, money, room);
					}
				...
				case 'berry1':
					...
					if (success) {
						...
						for (let c = 0; c < 30; c++) {
							let b = WL.getItem(berries1[Math.floor(Math.random() * berries1.length)]);
				...
				case 'berry2':
					...
					if (success) {
						...
						for (let c = 0; c < 30; c++) {
							let b = WL.getItem(berries2[Math.floor(Math.random() * berries2.length)]);
							if (!sggame.bag[b.slot][b.id]) sggame.bag[b.slot][b.id] = 0;
							sggame.bag[b.slot][b.id]++;
						}
						successfulTransaction(match, success, user, room);
