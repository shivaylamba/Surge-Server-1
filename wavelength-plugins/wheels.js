...
class Wheel {
	...
	endDisplay(winner, winnings) {
		...
		WL.nameColor(winner, true) + ' has won ' + winnings + '</div>';
	}
}
...
exports.commands = {
	wheel: {
		create: function (target, room, user) {
			if (room.id !== 'casino') return this.errorReply("This game may not be played outside of a casino.");
			...
			Economy.readMoney(user.userid, money => {
				...
				room.add('|raw|<b><font color="orange">' + WL.nameColor(user.name, true) + ' is hosting the wheel.</font></b>');
				room.update();
		...
		join: function (target, room, user) {
			...
			Economy.readMoney(user.userid, money => {
				...
				Economy.writeMoney(user.userid, (room.wheel.wheel[room.wheel.wheel.length - 1] * room.wheel.wheelMulti) * -1, function () {
					room.add('|raw|<b><font color="orange">' + WL.nameColor(user.name, true) + ' has spun the wheel.</font></b>');
			...
			delete room.wheel;
			room.add('|raw|<font color="orange">The WOM was ended by <b>' + WL.nameColor(user.name, true) + '</b>.</font>'); //html needed
		},
