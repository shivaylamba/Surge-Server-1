...
class PassTheBomb extends Rooms.RoomGame {
	...
	nextRound() {
		...
		this.release = setTimeout(() => {
			...
			this.room.add('|uhtmlchange|' + this.getMsg() + '<br><strong style = "font-size: 10pt;">The bomb has been passed to </strong>' + WL.nameColor(this.holder, true) + '</div>').update();
			this.canPass = true;
			this.resetTimer();
	...
	resetTimer() {
		this.timer = setTimeout(() => {
			let player = this.players.get(this.holder).name;
			this.room.add('|html|The bomb exploded and killed <span style = "' + WL.nameColor(this.holder, true) + '">' + player + '</span>').update();
	...
	removeUser(userid, left) {
		...
		this.room.add('|html|' + WL.nameColor(this.players.get(userid).name, true) + ' has ' + (left ? 'left' : 'been disqualified from') + ' the game.');
		this.players.delete(userid);
		this.madeMove = true;
		if (this.checkWinner()) {
			this.getWinner();
			...
			this.setBomb();
			let player = this.players.get(this.holder).name;
			this.room.add('|html|The bomb has been passed to ' + WL.nameColor(player, true) + '!').update();
		}
	}
	checkWinner() {
	...
	getWinner() {
		...
		if (user) {
			let msg = '<div class = "infobox"><center>This game of Pass the Bomb has been forcibly ended by ' + WL.nameColor(user.name, true) + '.</center></div>';
			if (!this.madeMove) {
...
let commands = {
	...
	proceed: function (target, room, user) {
		if (!room.passthebomb) return this.sendReply("There is no game of Pass The Bomb going on in this room.");
		if (!this.canTalk()) return this.errorReply("You cannot use this while unable to speak.");
		if (!user.can('ban', null, room)) return this.sendReply("You must be ranked @ or higher in this room to forcibly begin the first round of a game of Pass The Bomb.");
